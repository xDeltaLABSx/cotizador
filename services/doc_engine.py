...
# Tipografía ejecutiva corporativa limpia
FONT_NAME = "Segoe UI"
BODY_SIZE_PT = 10.0
TITLE_SIZE_PT = 11.5
...
def generar_cotizacion_docx(datos):
    """
    Genera el Word (.docx) con tus campos originales exactos pero con un diseño
    editorial y tipográfico pulido e idéntico al estándar de tus documentos de PC.
    """
    ...
    # --- 1. FECHA FORMAL (Con el espacio exacto bajo el membrete) ---
    ...
    _aplicar_estilo_parrafo(p_date, bold=True, space_after=10)
    
    # --- 2. BLOQUE DE DESTINATARIO ORIGINAL ---
    ...
    _aplicar_estilo_parrafo(p_client, bold=True, color_hex=COLORS["PRIMARY_HEX"], keep_next=True, space_after=10)
    
    # --- 3. SECCIÓN 1: OBJETIVO DEL PROYECTO ---
    h1 = doc.add_heading(level=2)
    h1.add_run("1. Objetivo del Proyecto")
    _aplicar_estilo_parrafo(h1, size_pt=TITLE_SIZE_PT, bold=True, color_hex=COLORS["PRIMARY_HEX"], keep_next=True, space_after=4)
    ...
    _aplicar_estilo_parrafo(h2, size_pt=TITLE_SIZE_PT, bold=True, color_hex=COLORS["PRIMARY_HEX"], keep_next=True, space_after=4)
    
    p_met = doc.add_paragraph(datos.get("metodologia", ""))
    _aplicar_estilo_parrafo(p_met, space_after=8)
    
    # --- 5. SECCIÓN 3: INSTRUMENTACIÓN Y EQUIPO ---
    h3 = doc.add_heading(level=2)
    h3.add_run("3. Instrumentación y Equipo Desplegado")
    _aplicar_estilo_parrafo(h3, size_pt=TITLE_SIZE_PT, bold=True, color_hex=COLORS["PRIMARY_HEX"], keep_next=True, space_after=4)
    
    p_eq = doc.add_paragraph(datos.get("equipo", ""))
    _aplicar_estilo_parrafo(p_eq, space_after=8)
    
    # --- 6. SECCIÓN 4: ENTREGABLES ---
    h4 = doc.add_heading(level=2)
    h4.add_run("4. Entregables del Proyecto")
    _aplicar_estilo_parrafo(h4, size_pt=TITLE_SIZE_PT, bold=True, color_hex=COLORS["PRIMARY_HEX"], keep_next=True, space_after=4)
    ...
    # --- 7. SECCIÓN 5: PROPUESTA ECONÓMICA (Tus 3 columnas originales con diseño fino) ---
    h5 = doc.add_heading(level=2)
    h5.add_run("5. Propuesta Económica")
    _aplicar_estilo_parrafo(h5, size_pt=TITLE_SIZE_PT, bold=True, color_hex=COLORS["PRIMARY_HEX"], keep_next=True, space_after=6)
    ...
    # --- 8. SECCIÓN 6: PREMISAS TÉCNICAS Y EXCLUSIONES ---
    h6 = doc.add_heading(level=2)
    h6.add_run("6. Premisas Técnicas y Exclusiones")
    _aplicar_estilo_parrafo(h6, size_pt=TITLE_SIZE_PT, bold=True, color_hex=COLORS["PRIMARY_HEX"], keep_next=True, space_after=4)
    ...
    for idx, excl in enumerate(exclusiones_lista):
        ...
        _aplicar_estilo_parrafo(p_ex, color_hex=COLORS["SECONDARY_HEX"], space_after=3)
    ...
    _aplicar_estilo_parrafo(h7, size_pt=TITLE_SIZE_PT, bold=True, color_hex=COLORS["PRIMARY_HEX"], keep_next=True, space_after=4)
    
    p_cl = doc.add_paragraph(datos.get("clausulas", ""))
    _aplicar_estilo_parrafo(p_cl, space_after=6)
    
    p_sal = doc.add_paragraph(datos.get("saludo_final", ""))
    _aplicar_estilo_parrafo(p_sal, space_after=10)
